...
export function createPromptBuilders(deps) {
  ...
  const buildCoreArticlePrompt = (job, settings, repairNote = "") => {
    ...
    const contentTypeNotes = {
      recipe: [
        ...
        "Social readers should understand by page 1 why this dish is worth cooking tonight, this week, or on the next repeat dinner cycle.",
      ...
      food_fact: [
        ...
        "Social readers should get the answer quickly, then understand exactly why it changes a real cook, shop, storage, or ingredient decision.",
    ...
    return [
      ...
      "- Assume many readers are landing cold from social. Orient them fast with a concrete promise, useful context, and no inside-baseball setup.",
      "- Write so a first-time visitor gets value quickly and a returning reader still finds a sharper detail, distinction, or kitchen judgment worth coming back for.",
      ...
      "- By the end of page 1, the reader should already have one useful answer or payoff and one clear reason to continue reading.",
  ...
  const buildSocialCandidatePrompt = (job, settings, article, selectedPages, preferredAngle = "", repairNote = "") => {
    ...
    return [
      ...
      "These candidates are meant to bring the right reader from social onto the blog, not just win shallow reactions.",
      "Write for two audiences at once: cold scrollers who need a concrete reason to click and returning readers who already expect useful low-hype kitchen judgment.",
      ...
      "Give enough clue to earn trust, but leave enough unresolved value that the full article still feels worth tapping.",
      ...
      "Make some candidates feel save-worthy because they promise a repeat-use kitchen lesson, and make some feel share-worthy because the reader can instantly picture who in their life needs it.",
      ...
      "The best candidates should feel like the blog has the fuller answer, cleaner explanation, or more dependable kitchen move, not like the social post already gave everything away.",
      ...
      "{",
      '  "social_candidates": [{"angle_key":"string","hook":"string","caption":"string","cta_hint":"string"}]',
      "}",
